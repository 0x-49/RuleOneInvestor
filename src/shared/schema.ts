import { pgTable, text, serial, real, jsonb, timestamp, varchar, index } from "drizzle-orm/pg-core";
...
export const insertStockSchema = createInsertSchema(stocks as any).omit({
  id: true,
  lastUpdated: true,
});

export const insertFinancialMetricsSchema = createInsertSchema(financialMetrics as any).omit({
  id: true,
}).required({
  stockId: true,
});

export const insertWatchlistItemSchema = createInsertSchema(watchlistItems as any).omit({
  id: true,
  addedAt: true,
});

export const insertValuationInputsSchema = createInsertSchema(valuationInputs as any).omit({
  id: true,
});
